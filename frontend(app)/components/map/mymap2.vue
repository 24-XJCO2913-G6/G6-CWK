...
<script module="ModuleInstance" lang="renderjs">
	export default {
		...
		computed:{
			total_center(){
				let x=0,y=0;
				let x_sum=this.routes.reduce((sum,i)=>sum+i.center[0],0)
				let y_sum=this.routes.reduce((sum,i)=>sum+i.center[1],0)
				console.log('total_center',[(x_sum/this.routes.length).toFixed(6),(y_sum/this.routes.length).toFixed(6)])
				return [(x_sum/this.routes.length).toFixed(6),(y_sum/this.routes.length).toFixed(6)]
			}
		},
		methods: {
			getColor(index){
			...
			initAmap() {
				...
				for(const [index, item] of this.routes.entries()){
					console.log('routes',this.routes);
					console.log('item',item)
				
					console.log('index',1)
					var polyline = new AMap.Polyline({
						path: item.path, // 设置路径数组
						strokeColor: this.getColor(index), // 设置线颜色为随机颜色
					...
					var marker = new AMap.Marker({
						position: item.path[item.path.length - 1], // 终点坐标
